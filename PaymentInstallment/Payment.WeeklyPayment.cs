...
namespace PaymentInstallment
{
    internal partial class Payment
    {
        ...
        public static void WeeklyPayment()
        {
            ...
            for (int i = 0; i < _planInput; i++)
            {
                Console.WriteLine("{0}\t{1}\t{2}\t{3}\t\t\t{4}\t{5}", i + 1, pay[i].models.Product, pay[i].models.date, pay[i].models.WeeklyPay, pay[i].models.NewWeeklyPay, pay[i].models.date.AddDays(_planInput));
